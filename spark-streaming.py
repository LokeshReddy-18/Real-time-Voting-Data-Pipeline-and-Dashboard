...
spark = (SparkSession.builder
         ...
         .config("spark.sql.streaming.checkpointLocation", "/Users/lokesh/Desktop/Data_Engineering/voting/Realtime_Voting_System/checkpoints")
         .config("spark.jars.packages", "org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.2")  # Spark-Kafka integration
         .getOrCreate())
...
votes_per_candidate = enriched_votes_df.groupBy("candidate_id", "candidate_name", "party_affiliation",
                                                "photo_url").agg(expr("sum(vote) as total_votes"))
...
votes_per_candidate_to_kafka = votes_per_candidate.selectExpr("to_json(struct(*)) AS value") \
    ...
    .option("checkpointLocation", "/Users/lokesh/Desktop/Data_Engineering/voting/Realtime_Voting_System/checkpoints/checkpoint1") \
    .outputMode("update") \
    .start()
...
turnout_by_location_to_kafka = turnout_by_location.selectExpr("to_json(struct(*)) AS value") \
    ...
    .option("checkpointLocation", "/Users/lokesh/Desktop/Data_Engineering/voting/Realtime_Voting_System/checkpoints/checkpoint2") \
    .outputMode("update") \
    .start()
